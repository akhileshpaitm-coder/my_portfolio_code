/**
 * Dashboard responsive audit: login, then check overflow at each viewport.
 */
...
const ROUTES = ["/dashboard", "/dashboard/profile", "/dashboard/settings"];
...
const main = async () => {
  ...
  for (const vp of VIEWPORTS) {
    ...
    // Login once per viewport (fresh session each time)
        ...
        if (overflowX > 1) {
          for (const el of document.querySelectorAll("body *")) {
            ...
            if (r.right > doc.clientWidth + 1 || r.left < -1) {
              // ignore the intentional off-canvas sidebar
  ...
  console.log(`\n${issues === 0 ? "DASHBOARD ALL CLEAN" : issues + " issue(s) found"}`);
  await browser.close();
};
